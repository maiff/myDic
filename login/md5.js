...
function rstr_hmac_md5(d, a) {
  var b = rstr2binl(d);
  if (b.length > 16) b = binl_md5(b, d.length * 8);
  var c = Array(16);
  d = Array(16);
  for (var e = 0; e < 16; e++) {
    c[e] = b[e] ^ 909522486;
...
function rstr2b64(d) {
  for (var a = "", b = d.length, c = 0; c < b; c += 3)
    for (var e = d.charCodeAt(c) << 16 | (c + 1 < b ? d.charCodeAt(c + 1) << 8 : 0) | (c + 2 < b ? d.charCodeAt(c + 2) : 0), f = 0; f < 4; f++) a += c * 8 + f * 6 > d.length * 8 ? b64pad : "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(e >>> 6 * (3 - f) & 63);
  return a
}

function rstr2any(d, a) {
  var b = a.length,
    c, e, f, g, h, i = Array(Math.ceil(d.length / 2));
  for (c = 0; c < i.length; c++) i[c] = d.charCodeAt(c * 2) << 8 | d.charCodeAt(c * 2 + 1);
  ...
  b = "";
  for (c = d.length - 1; c >= 0; c--) b += a.charAt(d[c]);
  return b
}
...
function str2rstr_utf8(d) {
  for (var a = "", b = -1, c, e; ++b < d.length;) {
    ...
    if (c <= 127) a += String.fromCharCode(c);
    else if (c <= 2047) a += String.fromCharCode(192 | c >>> 6 & 31, 128 | c & 63);
    else if (c <= 65535) a += String.fromCharCode(224 | c >>> 12 & 15, 128 | c >>> 6 & 63, 128 | c & 63);
    else if (c <= 2097151) a += String.fromCharCode(240 | c >>> 18 & 7, 128 | c >>> 12 & 63, 128 | c >>> 6 & 63, 128 | c & 63)
  }
  return a
}

function str2rstr_utf16le(d) {
  for (var a = "", b = 0; b < d.length; b++) a += String.fromCharCode(d.charCodeAt(b) & 255, d.charCodeAt(b) >>> 8 & 255);
  return a
}

function str2rstr_utf16be(d) {
  for (var a = "", b = 0; b < d.length; b++) a += String.fromCharCode(d.charCodeAt(b) >>> 8 & 255, d.charCodeAt(b) & 255);
  return a
}

function rstr2binl(d) {
  for (var a = Array(d.length >> 2), b = 0; b < a.length; b++) a[b] = 0;
  for (b = 0; b < d.length * 8; b += 8) a[b >> 5] |= (d.charCodeAt(b / 8) & 255) << b % 32;
  return a
}

function binl2rstr(d) {
  for (var a = "", b = 0; b < d.length * 32; b += 8) a += String.fromCharCode(d[b >> 5] >>> b % 32 & 255);
  return a
}
